...
@app.route("/privacy")
async def privacy():
    return send_file("static/privacy.pdf")
...
async def mainpage():
    ...
    try:
        ...
        print(
            "Failed to connect user to database. Trying again in 4 seconds", "warning"
        )
    ...
    resp = make_response(
        render_template(
            "home.html", msgs=messages, users=users_online, usersnum=users_online
        )
    )
...
async def validatelogin():
    ...
    try:
        ...
        return redirect(
            url_for(
                "login",
                message="Server failed to connect to database. Try again in a few seconds.",
            )
        )
    c = conn.cursor()
    c.execute(
        "SELECT * FROM usercred WHERE (username = %s OR email = %s) AND password = %s",
    ...
    settoken = result[2]
    c.execute(
        "SELECT veriemail FROM usercred WHERE (username = %s OR email = %s)",
    ...
    response.set_cookie("token", settoken, secure=True, expires=datetime.datetime.now() + datetime.timedelta(days=30))
    return response
...
async def validatesignup():
    ...
    if password == confirm_password:
        try:
            ...
            print(
                "Failed to connect user to database. Trying again in 4 seconds",
                "warning",
            )
            await asyncio.sleep(4)
            return redirect(request.url)
            ...
            if result is None:
                ...
                if len(username) < 2 or len(username) > 22:
                    ...
                    return redirect(
                        url_for(
                            "signup",
                            message="Username needs to be between 2 - 22 characters",
                        )
                    )
                ...
                if len(password) < 8 or len(password) > 52:
                    ...
                    return redirect(
                        url_for(
                            "signup",
                            message="Password need to be between 8 - 52 characters",
                        )
                    )
                ...
                hashed_password = h.hexdigest()
                c.execute(
                    "INSERT INTO usercred (username, password, token, email) VALUES (%s, %s, %s, %s)",
                    [username.lower(), hashed_password, sign_up_token, email],
                )
                email_token = token_generator()
                c.execute(
                    "INSERT INTO emailtokens (token, email) VALUES (%s, %s)",
                    [email_token, email],
                )
                conn.commit()
                conn.close()
                ...
                return redirect(
                    url_for(
                        "login", message="Check your email for a verification message"
                    )
                )
...
async def verifyemail():
    ...
    if request.args.get("token") is None:
        ...
        if result is None:
            ...
            c.execute(
                "UPDATE usercred SET veriemail = 'YES' WHERE email = %s", [result[1]]
            )
            conn.commit()
            conn.close()
            return redirect(
                url_for(
                    "login", message="Your email has been verified. You can now log in."
                )
            )
...
def handel_disconnect():
    try:
        ...
        socketio.emit(
            "error",
            "Server could not contact database, Try again in a few seconds",
            room=request.sid,
        )
    global users_online
...
def handle_message():
    ...
    if token == "":
        ...
        c.execute(
            "SELECT username FROM usercred WHERE token = %s", [str(data["token"])]
        )
        username = c.fetchone()[0]
        if username == "":
